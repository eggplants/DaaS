...
def reading(text: str) -> str:
    ...
    # convert with dict
    with Path(config.READING_DICT_FILE_PATH).open(mode="r") as f:
        reader = csv.reader(f)
...
def preprocessing(text: str) -> str:
    ...
    result = re.sub(
        r"[a-z]+",
        lambda m: "" if re.match(r"^w+$", m.group(0), re.IGNORECASE) else m.group(0),
        result,
        flags=re.IGNORECASE,
    )
    ...
    # remove emoji
    return "".join(ch for ch in result if ch not in emoji.EMOJI_DATA)
...
def normalize(text: str) -> str:
    ...
    return re.sub(r"(.)\1{2,}", r"\1", result)
